...
const AdminHome = () => {
  const { user } = useAuth();
  const { data: stats, isLoading: statsLoading } = useFetchData(
    "adminStats",
    "/admin/stats",
    {},
    true
  );
  const { data: orderStats, isLoading: orderStatsLoading } = useFetchData(
    "adminOrderStats",
    "/admin/order-stats",
    {},
    true
    ...
    <div className="font-inter">
      ...
      {statsLoading ? (
        ...
        <div className="grid grid-cols-2 gap-6">
          <StatsCard
            svg={revenueSVG}
            css="from-[#BB34F5] to-[#FCDBFF]"
            count={stats?.revenue || 0}
            label="Revenue"
          />
          <StatsCard
            svg={customersSVG}
            css="from-[#D3A256] to-[#FDE8C0]"
            count={stats?.customers || 0}
            label="Customers"
          />
          <StatsCard
            svg={productsSVG}
            css="from-[#FE4880] to-[#FECDE9]"
            count={stats?.products || 0}
            label="Products"
          />
          <StatsCard
            svg={ordersSVG}
            css="from-[#6AAEFF] to-[#B6F7FF]"
      ...
      {!orderStatsLoading && (
        <div className="flex gap-6 items-center *:h-96 *:w-full">
          <div>
            <ResponsiveContainer>
              <BarChart
                width={500}
